    ...
    const renderItem = ({ item }) => (
        <PixelCard style={[styles.rewardItem, { backgroundColor: theme.surface, borderColor: theme.border }]}>
            <View style={styles.rewardInfo}>
                <Text style={[styles.rewardTitle, theme.typography.h3, { color: theme.text }]}>{item.name}</Text>
                <Text style={[styles.rewardCost, theme.typography.bodyBold, { color: theme.warning }]}>{item.cost} {t.gold.toUpperCase()}</Text>
            </View>
            <TouchableOpacity
    ...
    return (
        <View style={[styles.container, { backgroundColor: theme.background }]}>
            <View style={styles.header}>
                <Text style={[styles.headerTitle, theme.typography.h1, { color: theme.textLight }]}>{t.marketplace}</Text>
            </View>
            ...
            <FlatList
                ...
                ListEmptyComponent={<Text style={[styles.emptyText, theme.typography.h3, { color: theme.text }]}>{t.noItemsInStock}</Text>}
            />
        </View>
    );
};
...
const styles = StyleSheet.create({
    ...
    rewardCost: { marginTop: 4, fontWeight: 'bold' },
    buyButton: {
        paddingVertical: 8,
