    ...
    const renderInventoryItem = ({ item }: { item: any }) => {
        ...
        return (
            ...
            >
                ...
                <Text style={[styles.useText, { color: theme.primary }]}>{t.use || 'USE'}</Text>
            </TouchableOpacity>
        );
    };
        ...
        >
            <View style={styles.modalOverlay}>
                <PixelCard style={[styles.modalContent, { backgroundColor: theme.surface, borderColor: theme.border }]}>
                    <Text style={[styles.modalTitle, { color: theme.text }]}>{t.inventory || 'INVENTORY'}</Text>

                    {populatedInventory.length === 0 ? (
                        <Text style={[styles.emptyText, { color: theme.text }]}>No consumables available</Text>
                    ) : (
                        <FlatList
